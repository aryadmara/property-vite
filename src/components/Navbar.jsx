...
function Navbar() {
    const [view, setView] = useState ("invisible md:visible")
    ...
    return (  
        <header className=" absolute w-[80rem] m-auto left-0 right-0 my-7 flex items-center justify-between  text-white">
            {/* <p>{name}</p> */}
        <div className={`${view} text-black bottom-24 right-4 fixed bg-white shadow rounded-md z-50`} > 
        ...
        <div className=" bottom-4 right-4 fixed bg-[#4eca5c] rounded-full p-4 z-50 cursor-pointer">
            <span className="  rounded-full"><FaWhatsapp className="  w-8 h-8"/></span>
        </div>
